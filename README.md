...
`$dask-scheduler`. A scheduler url will show up below, copy it.  
Then open 2 terminals for both nodes, run `$dask-worker <scheduler url>` and keep all 3 terminals on hold.  
Open another terminal for node 0, run `$python3 FPGA-MD/InterFPGA_MD/Notebooks/run.py <scheduler url> <dump group> <number of iterations>`  
...
## For Developers
To compile, run `compile.sh` in FPGA-MD/InterFPGA_MD/ to compile the design
## Architecture
